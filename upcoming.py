...
BET_SIZE = 100  #dinheiro a apostar
...
with open('upcoming1.txt', 'w') as f:
...
with open('upcoming2.txt', 'w') as file:
    for event in arbitrage_events:
        event.calculate_arbitrage_bets()
        event.convert_decimal_to_american()
...
for event in arbitrage_events:
    ...
    for index, outcome in enumerate(event.best_odds):
        row += [outcome[BOOKMAKER_INDEX], outcome[NAME_INDEX], outcome[ODDS_INDEX], event.bet_amounts[index]]
    while len(row) < len(my_columns):
        row.append('N/A')
    dataframe.loc[len(dataframe.index)] = row


with pd.ExcelWriter('bets.xlsx') as writer:
...
if len(arbitrage_events) != 0:
    df = pd.read_excel('upcoming_events_bets.xlsx')

    for column in ['Name #1', 'Name #2']:
        teams.extend(df[column].dropna().tolist())
    print(teams) #['Sporting Lisbon', 'Vitória SC']

    def to_hashtag(team_name):
        name = re.sub(r'\W+', '', team_name)
        name = unidecode(name)
        return f"#{name.lower()}" # Convert to lowercase and prepend '#'

    hashtags = [to_hashtag(team) for team in teams]
    #print(hashtags)  # Output: ['#sportinglisbon', '#vitoriasc', '#limoraesp']
